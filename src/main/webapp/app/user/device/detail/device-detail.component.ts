...
import {DeviceMonitor} from "../device-monitor.model";
...
import {DeviceTimerModalComponent} from "./timer-modal/device-timer-modal.component";
...
export class DeviceDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.device !== null && this.device?.deviceType == "CONTROL") {
      this.deviceFirebaseService.getDeviceStateById()
        .doc("device_action")
        .collection(<string>this.device?.createdBy)
        .doc(this.device.id!!).valueChanges().subscribe(data => {
  ...
  onChangeStateDevice() {
    ...
    if (this.device !== null) {
      this.deviceService.updateStateDevice(this.device.id!!).subscribe(
        data => {
          console.log(data);
  ...
  loadAll(): void {
    if (this.device?.id !== null && this.device?.deviceType === "MONITOR") {
      this.isLoading = true;
      this.deviceMonitorService.query({
        page: this.page - 1,
        size: this.itemsPerPage,
        sort: this.sort(),
      }, this.device?.id).subscribe({
